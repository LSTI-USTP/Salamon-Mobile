...
public class Teclado extends Activity implements OnClickListener, OnItemSelectedListener, DefaultDatas, OnReciveTranfer
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		setContentView(R.layout.layout_teclado);
		...
		{
			case CARTA:
				this.titleTeclado.setText(R.string.numeroCarta);
				this.tvReadInputText.setHint(R.string.numeroCarta);
				if(oldValues != null)
					this.wait = oldValues.length() +1;
				break;
			case MATRICULA:
				this.titleTeclado.setText(R.string.matricula);
				this.tvReadInputText.setHint(R.string.matricula);
				if(this.oldValues != null && oldValues.length()>0)
				{
					String campos[] = oldValues.split(" ");
					if(campos[0].equals("STP"))
					{
						//  1 23 45 6...
						//STP 56-09 K
						//12345678901
						if(campos[0].equals("STP"))
						{
							this.wait = (oldValues.length()<3)? 1
									:(oldValues.length() <= 4)? 2
									:(oldValues.length() <= 5)? 3
									:(oldValues.length() <= 7)? 4
									:(oldValues.length() <= 8)? 5
									:(oldValues.length() <= 10)?6
									:6+oldValues.length()-10;
							this.readMatricula = "STP";
							
						}
						else wait = oldValues.length();
					}
				}
				break;
		}
	...
	private void init()
	{
		this.spinerVaforitos = (Spinner) findViewById(R.id.spinnerTecladoFavoritos);
		...
		DefaultItemList.generateItems(adapter, ItemTypes.SPINNER_ITEM_FAVORITS, 
				new DualValues<Integer, String>(1, "FAVORITO"),
				new DualValues<Integer, String>(1, "STP"),
				new DualValues<Integer, String>(1, "IT"),
				new DualValues<Integer, String>(1, "FMD"),
				new DualValues<Integer, String>(1, "UPDE"),
				new DualValues<Integer, String>(1, "-"),
				new DualValues<Integer, String>(1, " "));
		
		this.spinerVaforitos.setAdapter(adapter);
		this.spinerVaforitos.setOnItemSelectedListener(this);
	}
